...
def generate_jwt(payload):
    ...
    claims = {"iat": int(time.time()), "jti": str(uuid.uuid4()), "payload": payload}
    ...
    signature = hmac.new(
        SECRET_KEY, encoded_header + b"." + encoded_claims, hashlib.sha512
    ).digest()
...
async def proxy_handler(request):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(
            UPSTREAM_URL, data=content, headers=headers
        ) as response:
            ...
            return web.Response(
                body=response_content, status=response.status, headers=response.headers
            )
...
async def status_handler(request):
    ...
    status_data = {"time_from_start": time_from_start, "request_count": request_count}
